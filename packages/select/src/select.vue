<template>
  <div
    ...
    <transition
      ...
      <ex-select-menu style="height: 204px;"
        ref="popper"
        :append-to-body="popperAppendToBody"
        v-show="visible && emptyText !== false">
        <RecycleScroller class="scroller el-select-dropdown__list el-select-dropdown__wrap" tag="ul"
                         :items="filterNodes" :item-size="32" key-field="value" v-slot="{ item, index }"
                         v-show="options.length > 0 && !loading" :emitUpdate="true">
          <ex-option :value="query" created v-if="showNewOption">
          </ex-option>
          <ex-option :label="item.label" :value="item.value">
          </ex-option>
        </RecycleScroller>
        <!--<scrollbar-->
          <!--:items="filterNodes"-->
...
<script type="text/babel">
  ...
  import ExSelectMenu from './select-dropdown.vue';
  import ExOption from './option.vue';
  ...
  export default {
    ...
    watch: {
      ...
      visible(val) {
        if (!val) {
          this.broadcast('ExSelectDropdown', 'destroyPopper');
          if (this.$refs.input) {
          ...
          this.searchKey = ''
          this.broadcast('ExSelectDropdown', 'updatePopper');
          if (this.filterable) {
    ...
    methods: {
      ...
      handleQueryChange(val) {
        ...
        this.previousQuery = val;
        this.$nextTick(() => {
          if (this.visible) this.broadcast('ExSelectDropdown', 'updatePopper');
        });
        this.hoverIndex = -1;
        ...
        if (this.remote && typeof this.remoteMethod === 'function') {
          ...
          this.broadcast('ExOption', 'queryChange', val);
          this.broadcast('ExOptionGroup', 'queryChange');
        }
        if (this.defaultFirstOption && (this.filterable || this.remote) && this.filteredOptionsCount) {
          this.checkDefaultFirstOption();
